...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = React.memo(
    ({
        ...
        const toggleMetaExpansion = useCallback((rowIndex: number, event: React.MouseEvent) => {
        ...
        const expandAllMeta = useCallback(() => {
        ...
        const collapseAllMeta = useCallback(() => {
        ...
        // Calculate paginated data - memoized to avoid recalculation on every render
        ...
        const paginatedHistory = useMemo(() => backtestResult?.history?.slice(startIndex, endIndex) || [], [backtestResult?.history, startIndex, endIndex]);
                        ...
                        <div ref={exportableRef}>
                            ...
                            <div className="mb-6 pb-4 border-b border-tuna-600">
                                ...
                                <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-2 text-sm">
                                    ...
                                    <div>
                                        <span className="text-tuna-400">Starting Amount:</span>{" "}
                                        <span className="font-medium">${backtestResult.startingAmount.toLocaleString()}</span>
                                    </div>
                                    <div>
                                                    ...
                                                    <tr key={globalIndex}>
                                                        ...
                                                        <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium tracking-wider">
                                                            {strategyResult?.price !== undefined ? (
                                                                currency(strategyResult.price).format()
                                                            ) : (
                                                                <span className="text-tuna-400">-</span>
                                                            )}
                                                        </td>
                                                        ...
                                                        <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium tracking-wider">
                                                            {(() => {
                                                                ...
                                                                const previousPortfolioValue =
                                                                    index > 0
                                                                        ? backtestResult.history[index - 1].portfolioSnapshot.portfolioValue
